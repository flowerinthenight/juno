...
fn api_create_topic(rt: &Runtime, mut stream: TcpStream, client: &Client, topic: &str) -> Result<()> {
...
fn api_delete_topic(rt: &Runtime, mut stream: TcpStream, client: &Client, topic: &str) -> Result<()> {
    let (tx_rt, rx_rt): (Sender<String>, Receiver<String>) = unbounded();
    rt.block_on(async {
        let mut q = String::new();
...
fn api_create_sub(rt: &Runtime, mut stream: TcpStream, client: &Client, line: &str) -> Result<()> {
...
fn api_publish_msg(rt: &Runtime, mut stream: TcpStream, client: &Client, line: &str) -> Result<bool> {
    let vals: Vec<&str> = line.split(" ").collect();
    if vals.len() < 2 {
        let mut err = String::new();
        write!(&mut err, "-Invalid payload format\n")?;
